...
def bufferbloat():
    ...
    topo = BBTopo()
    topo.build() #added this here because the build function wasn't being called
    ...
    h2 = net.get('h2')
    h1 = net.get('h1')
    fetch_times = [] # list of the float fetch times
    start_time = time()
    while True:
        # do the measurement (say) 3 times.
        sleep(5)
        #Piazza post says 3 times every 5 seconds, not sure if it's supposed to be spaced out somehow
        result = h2.cmd("curl -o /dev/null -s -w %{time_total} " + h1.IP() + "/http/index.html")
        fetch_times.append(float(result))
        result = h2.cmd("curl -o /dev/null -s -w %{time_total} " + h1.IP() + "/http/index.html")
        fetch_times.append(float(result))
        result = h2.cmd("curl -o /dev/null -s -w %{time_total} " + h1.IP() + "/http/index.html")
        fetch_times.append(float(result))
    ...
    # Compute the average
    average = avg(fetch_times) # sum/number of entries.
    print "Average: " + str(average)

    # Compute the standard deviation
    std_dev = stdev(fetch_times)
